    ...
    return (
      <>
        ...
        <div className="flex flex-col gap-6 mt-48">
          ...
          <Link className="text-xl hover:text-blue-700 font-bold" to={"/login"}>
            Go to Login Page.
          </Link>
        </div>
      </>
  ...
  return (
    ...
    <div className="h-auto justify-center items-center flex flex-col">
      ...
      <div className="flex justify-center h-96 items-center flex-wrap mt-24">
        {blogs?.length === 0 && (
          <div>
            <h1 className="text-2xl">No blogs to show.</h1>
          </div>
        )}
        {blogs?.length >= 0 && (
          <div className="flex gap-24 justify-center flex-wrap ">
            {blogs?.map((blog) => (
              <div
                key={blog?._id}
                className="border-2 h-96 p-5 flex gap-3 flex-col w-40rem mx-12 rounded-lg"
              >
                <div className="">
                  <div className="flex justify-between items-center">
                    <div className="flex gap-2">
                      ...
                      />
                      <p
                        className="text-2xl text-lime-800 font-bold border-2 px-3 cursor-pointer py-1 rounded-full"
                        onClick={() =>
                          navigate(`/user-profile/${blog?.createdBy?._id}`)
                        }
                      >
                        {blog?.createdBy?.name}
                      </p>
                ...
                <div className="flex">
                  <div>
                    <p className="font-bold text-lime-700 px-2">
